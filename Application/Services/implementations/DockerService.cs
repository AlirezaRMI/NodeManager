...
public class DockerService(IDockerClient dockerClient, ILogger<IDockerService> logger) : IDockerService
{
    ...
    public async Task OpenFirewallPortAsync(int port, string protocol = "tcp") => await ExecuteShellCommandAsync("sudo", $"ufw allow {port}/{protocol}");
    public async Task CloseFirewallPortAsync(int port, string protocol = "tcp") => await ExecuteShellCommandAsync("sudo", $"ufw delete allow {port}/{protocol}");
    public async Task WriteFileOnHostAsync(string filePath, string content)
    {
        var tempFilePath = Path.GetTempFileName();
        await File.WriteAllTextAsync(tempFilePath, content);
        try { await ExecuteShellCommandAsync("sudo", $"bash -c \"cat {tempFilePath} > {filePath}\""); }
        finally { File.Delete(tempFilePath); }
    }

    private async Task<(int ExitCode, string StdOut, string StdErr)> ExecuteShellCommandAsync(string command, string arguments)
    {
        var process = new Process { StartInfo = new ProcessStartInfo { FileName = "/bin/bash", Arguments = $"-c \"{command} {arguments}\"", RedirectStandardOutput = true, RedirectStandardError = true, UseShellExecute = false, CreateNoWindow = true, } };
        process.Start();
        string stdout = await process.StandardOutput.ReadToEndAsync();
        string stderr = await process.StandardError.ReadToEndAsync();
        await process.WaitForExitAsync();
        if (process.ExitCode != 0) { logger.LogWarning("Shell command '{Cmd} {Args}' failed with code {Code}. Stderr: {Err}", command, arguments, process.ExitCode, stderr); }
